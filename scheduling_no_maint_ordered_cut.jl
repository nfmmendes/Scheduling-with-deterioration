        ...
        open(string("Instances/Instances With Deterioration/Instances/",instanceName)) do instance
           ...
           @variable(model, Cmax, base_name="Cmax")
			...
			for i in 2:(NUMBER_OF_JOBS+1)
			  ...
			  @constraint(model, sum(a[j,m] for j = 2:(NUMBER_OF_JOBS+1) ) - sum(p[j-1,m]*x[i,j,m] for i=1:(NUMBER_OF_JOBS+1), j=2:(NUMBER_OF_JOBS+1)) -
 								sum(t[m]*x[1,j,m] for j=2:(NUMBER_OF_JOBS+1) ) <= 0)
		  	end
